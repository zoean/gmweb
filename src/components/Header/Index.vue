  ...
  >
    ...
    >
      <el-form :model="form">
        ...
        <el-form-item label="手机号" :label-width="formLabelWidth">
          <el-input
            :value="$store.state.accountNumber"
            style="width: 80%"
            disabled
            size="small"
          ></el-input>
        ...
        <el-form-item label="验证码" :label-width="formLabelWidth">
          ...
          <el-button
            ...
            "
            plain
            @click="sendDingVerCode"
            :disabled="form_rule_flag"
            >{{ form_rule_name }}</el-button
          >
        </el-form-item>

        <el-form-item label="" :label-width="formLabelWidth">
          <div style="font-size: 12px; float: right; margin-right: 20%">
            @：获取验证码后，请去钉钉查看
          </div>
        </el-form-item>
      </el-form>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="dialoghold" size="small"
          >保 存</el-button
        >
        <el-button @click="dialog_cancel" v-if="!oneLogin" size="small"
          >取 消</el-button
        >
        <el-button @click="dialog_cancel_back" v-if="oneLogin" size="small"
          >返回登录页</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button
          type="primary"
          @click="dialoghold_phone('form_phone')"
          size="small"
          >保 存</el-button
        >
        <el-button @click="dialog_cancel_phone" size="small" plain
          >取 消</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      <el-form :model="form_wx" :rules="rules_wx" ref="form_wx">
        ...
        >
          ...
          <div style="width: 120px; float: right; margin-right: 20%">
            <img
              src="../../assets/images/wx_ma.jpg"
              style="width: 120px; height: 120px"
              alt=""
            />
            <div style="height: 30px; line-height: 0px; text-align: center">
              二维码示例
            </div>
          </div>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="dialoghold_wx('form_wx')" size="small"
          >保 存</el-button
        >
        <el-button @click="dialog_cancel_wx" size="small" plain
          >取 消</el-button
        >
      </span>
    </el-dialog>
      ...
      <el-col :span="18" style="height: 60px !important">
        <el-row type="flex" justify="start">
          <el-col :span="5" style="min-width: 300px">
            <div
              class="index-hleft"
              @click="iconTitleClick"
              :class="back_Change ? 'back_Change' : ''"
            >
              <el-image
                style="width: 130px; position: relative; top: 2px"
                :src="
                  back_Change
                    ? require('../../assets/images/logo_Name_A.png')
                    : require('../../assets/images/logo_Name_A.png')
                "
                fit="fit"
              ></el-image>
      ...
      <el-col :span="6" style="min-width: 200px">
        <el-dropdown>
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item @click.native="change_password"
              >设置密码</el-dropdown-item
            >
            <el-dropdown-item @click.native="change_wx"
              >设置微信</el-dropdown-item
            >
            <el-dropdown-item @click.native="change_phone"
              >设置工作手机</el-dropdown-item
            >
          ...
          >
            <el-tab-pane :label="`未读 (${notReadNumValue})`" name="first">
              <dl v-for="(item, index) in unReadList" :key="index">
                <dt title="点击查看消息内容" @click="handleLookClick(item)">
                  <span></span
                  >{{
                    `${item.type}${item.fold ? maxSlice(item.msg) : item.msg}`
                  }}
                </dt>
                <dd>{{ item.createTime }}</dd>
              </dl>
              <dl
                v-if="!unReadList.length && !fullscreenLoading"
                class="no-data"
              >
                <dt class="no-data">暂无未读消息</dt>
              </dl>
            </el-tab-pane>
            <el-tab-pane label="已读" name="second">
              <dl v-for="(item, index) in fullReadList" :key="index">
                <dt title="点击查看消息内容" @click="item.fold = !item.fold">
                  {{
                    `${item.type}${item.fold ? maxSlice(item.msg) : item.msg}`
                  }}
                </dt>
              ...
              <dl
                v-if="!fullReadList.length && !fullscreenLoading"
                class="no-data"
              >
                <dt class="no-data">暂无已读消息</dt>
              </dl>
            </el-tab-pane>
          </el-tabs>
          <el-badge
            slot="reference"
            :value="notReadNumValue"
        ...
        <a
          class="guide"
          href="https://www.jhwx.com/staff_open.php"
          target="_blank"
          >使用教程</a
        >
      </el-col>
...
export default {
  ...
  methods: {
    ...
    maxSlice(msg) {
      return msg.length > this.maxLength
        ? `${msg.slice(0, this.maxLength)}...`
        : msg;
    },
    handleLookClick(row) {
      row.fold = !row.fold;
    ...
    dialoghold() {
      let type = "";
      if (
        !pass_word.test(this.form.password2) ||
        !this.form.password1 ||
        !this.form.password2
      ) {
        this.$message({
    ...
    getProfile() {
      this.$smoke_get(getProfile, {}).then((res) => {
        if (res.code == 200) {
          ...
          this.form_wx.wxQrcode = this.imageUrl =
            "/file_api" + "/" + res.data.wxQrcode;
        }
      });
    },
    upProfile_phone() {
      ...
      this.$smoke_post(getUserLoginMessage, {}).then((res) => {
        if (res.code == 200) {
          ...
          localStorage.setItem(
            "userMenuList",
            JSON.stringify(
              menuNumberFunc(res.data.userMenuList, this.clueDataNumberList)
            )
          );
...
.el-header /deep/ div.el-dialog__body {
  height: auto;
}
.el-header {
  background: #fff;
